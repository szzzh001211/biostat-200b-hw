...
**All the observations with leverage larger than $3\bar{h}$ are unusual. $\bar{h}$ is 0.0354 so $3\bar{h}$ is 0.1062. So observations with IDs 104, 112 have high leverage. The values are 0.1525 and 0.1523, respectively.**

The studentized residual plot is as follows:
<p align="center">
<img src="./screenshot/截屏2024-02-22 10.57.34.png" width="80%"/>
</p>
**All the observations with studentized residual larger than 2 or smaller than -2 are unusual. So observations with ID 47 is unusual and it has the highest studentized residual and the value is 4.0883. Observations 33, 43, 54, 26, and 103 are unusual.**
...
<p align="center">
<img src="./screenshot/截屏2024-02-27 10.46.04.png" width="80%"/>
</p>

The new component-plus-residual plot for `census` is as follows:
<p align="center">
<img src="./screenshot/截屏2024-02-26 19.45.51.png" width="80%"/>
</p>
...
<p align="center">
<img src="./screenshot/截屏2024-02-26 19.48.31.png" width="80%"/>
</p>
$$
log(length) = 0.91575 + 0.11325 \times log(census) + 0.00973 \times age + 0.00314 \times xray
$$
...
### Answer:
After creating the new variable `nuratio` = (`nurses`/`census`), we check the distribution of all predictors. The distribution of `beds`, `length` and `nuratio` is right-skewed, and we log-transform it.
...
Here we use best subset selection to select the best model. The best model is the one with the smallest Cp, AIC, and SBC and we arrange the best 8 models according to Cp. So the first row is the predictors in the best model, `log(beds)`, `xray`, `log(length)` and `log(nuratio)`. The best model is as follows:

\begin{align*}
risk &= -5.784931 + 0.509710 \times log(beds) + 0.016694 \times xray + 2.757251 \times log(length) + \\
&\quad 1.040605 \times log(nuratio)
\end{align*}
